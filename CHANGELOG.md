...
- **The Mermaid loader now actually overrides Hextra's, and pins the Mermaid version instead of tracking `@latest`.** `docs-theme-extras` shipped its Mermaid loader at `layouts/partials/scripts/mermaid.html`, but Hextra v0.12+ resolves partial overrides from `_partials/` — so the file was silently shadowed and **Hextra's built-in loader ran instead** on every consumer, which also meant extras' dark-theme actor/note color fix (added so `sequenceDiagram` participant labels stay legible) was dead the whole time. The loader moved to `layouts/_partials/scripts/mermaid.html`, where it wins over Hextra's (the same chain that lets the centralized sidebar/TOC override Hextra). Separately, both the extras and Hextra defaults loaded `https://cdn.jsdelivr.net/npm/mermaid@latest/dist`, so every visitor pulled whatever version jsdelivr was currently serving at their CDN edge — meaning a render regression in a freshly published `@latest` is invisible to anyone holding an older cached copy and impossible to reproduce on demand. The default is now pinned to `mermaid@11.15.0` (verified to render the agentgateway diagrams correctly); the loader still self-copies, fingerprints, and SRIs the file, and a consumer can still override with `params.mermaid.base`/`params.mermaid.js`. Example of a diagram that intermittently fails to render (the "Challenge 1" flowchart): [Solo Enterprise for agentgateway — Token exchange](https://docs.solo.io/agentgateway/latest/security/token-exchange/).

- **Mermaid diagrams now wait for the tab to actually be painted before rendering, fixing the diagram that collapses to a tiny image in the corner of an empty box.** Mermaid sizes an SVG from the text bounding box it measures at render time and bakes that into an inline `max-width`. The old loader used `startOnLoad: true`, so every diagram rendered on `DOMContentLoaded` regardless of whether the page was being painted. When a page loads in an inactive/background browser tab (a Cmd-click, an "open in new tab", a restored session), `getBBox()` returns 0, so Mermaid bakes a permanent `max-width: 16px` / `16×16` viewBox into the SVG — a tiny diagram stuck in the top-left of a full-size box that never recovers once the tab is focused. This reads as a Chrome-only, "can't reproduce it" bug because it depends on *how* the page was opened. The loader now uses `startOnLoad: false` and defers the first render to a `requestAnimationFrame` — rAF callbacks are paused in background tabs, so a diagram opened in one waits until the tab is foregrounded and laid out before it is measured; a page that loads hidden renders on its first `visibilitychange`. All diagrams still render in a single pass once visible (no scroll-gating), so print/"Save as PDF" and the test contract are unaffected. Render is also gated on `document.fonts.ready` so text is sized at its final width, and `startOnLoad: false` closes a latent race in which the `dataset.original` capture (used for the dark/light re-render) could store an already-rendered SVG instead of the graph source. Caveats: the failure is intermittent, and the background-tab trigger is *inferred* from the identical zero-bounding-box mechanism reproduced with a hidden container — not from a directly reproduced throttled tab; a diagram inside a `display:none` container is not covered by this fix (it was not the reported case and was not handled before either). Example of the affected diagram: [Solo Enterprise for agentgateway — Token exchange](https://docs.solo.io/agentgateway/latest/security/token-exchange/).
