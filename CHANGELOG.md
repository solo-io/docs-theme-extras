...
**Verified** the redirect directly (`curl -sI https://docs.solo.io/` → `301` → `https://www.solo.io/docs` → `200`) and the blast radius by grepping the docs hub's configs: 7 of the 8 `hugo-<product>.toml` files set `params.sidebar.logo` (agentgateway is the exception, it dropped the param deliberately) and none of the 8 sets `navbar.logo.link`, so all 7 take this default today.
...
**Consumer action.** None beyond the pin bump. Sites that set `params.navbar.logo.link` are unaffected; sites with no `sidebar.logo` are unaffected.
