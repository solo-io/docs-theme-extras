...
- **Mermaid diagrams now render lazily once they become visible, fixing the diagram that collapses to a tiny image in the corner of an empty box.** Mermaid sizes an SVG from the text bounding box it measures at render time and bakes that into an inline `max-width`. The old loader used `startOnLoad: true`, so every diagram rendered on `DOMContentLoaded` regardless of whether it was laid out. When a page renders in an inactive/background browser tab (a Cmd-click, an "open in new tab", a restored session) or inside a hidden container, `getBBox()` returns 0, so Mermaid bakes a permanent `max-width: 16px` / `16×16` viewBox into the SVG — a tiny diagram stuck in the top-left of a full-size box that never recovers once the tab is focused. This reads as a Chrome-only, "can't reproduce it" bug because it depends on *how* the page was opened. The loader now uses `startOnLoad: false` plus an `IntersectionObserver` that renders each diagram only once it scrolls into view: an IO callback fires only while the page is actually painting, so a diagram in a background tab waits until the tab is foregrounded and laid out before it is measured. Render is gated on `document.fonts.ready` so text is sized at its final width, a `beforeprint` pass force-renders any still-deferred diagrams so "Save as PDF" isn't missing them, and `startOnLoad: false` also closes a latent race in which the `dataset.original` capture (used for the dark/light re-render) could store an already-rendered SVG instead of the graph source. Caveats: the failure is intermittent, and the background-tab trigger is *inferred* from the identical zero-bounding-box mechanism reproduced with a hidden container — not from a directly reproduced throttled tab. Example of the affected diagram: [Solo Enterprise for agentgateway — Token exchange](https://docs.solo.io/agentgateway/latest/security/token-exchange/).
