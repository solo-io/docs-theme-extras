...
const BOOK_PATH = process.env.PDF_BOOK_PATH || "/docs/book.html";
...
async function main() {
  ...
  const bookURL = `http://127.0.0.1:${port}${BOOK_PATH}`;
  ...
  try {
    ...
    // <ul>/<li> nesting (already captured above) is at risk.
    const outlineTree = await page.evaluate((skeleton) => {
