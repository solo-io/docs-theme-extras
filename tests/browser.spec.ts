...
test.describe("mobile drawer: AJAX section/version swap", () => {
  ...
  // The whole point of the bindDrawer() refactor is that the chip handlers are
  // re-attached to the markup an AJAX swap writes in. A single swap does not
  // prove that: the first swap is driven by the listeners bound on DOMContentLoaded.
  // Only a SECOND swap exercises the re-bind, so a regression that drops it
  // (e.g. moving bindDrawer back out of the .then) would otherwise ship green.
  test("a second version chip tap still swaps — the re-bound handlers survive", async ({ page }) => {
    test.skip(target.versions.length < 3, "needs 3+ versions for two distinct hops");
    const [a, b] = target.versions.filter((v) => !EVERYTHING.includes(`/${v}/`));
    ...
    // Assert on the active chip's href, not its text: the chip label is the
    // configured `dropdown` string ("main (dev)"), which need not contain the
    // version slug.
    const activeHref = panel.locator(".sidebar-mobile-version-active");

    await panel.locator(`.sidebar-mobile-version-link[href*="/${a}/"]`).first().click();
    await expect(activeHref).toHaveAttribute("href", new RegExp(`/${a}/`));

    // Second hop, driven entirely by listeners bound after the first swap.
    await panel.locator(`.sidebar-mobile-version-link[href*="/${b}/"]`).first().click();
    await expect(activeHref).toHaveAttribute("href", new RegExp(`/${b}/`));

    expect(page.url(), "second swap navigated instead of swapping in place").toBe(startUrl);
    await expect(panel).toHaveClass(/mobile-sidebar-open/);
  });

  // A swap changes only the drawer — the page, its URL and the navbar version
  // dropdown still belong to the version the reader started on. That divergence
  // must not outlive the drawer: closing without picking a topic is a cancel, so
  // reopening has to show the page's OWN tree, not the one that was browsed to.
  test("closing the drawer discards the swap instead of leaving a stale tree", async ({ page }) => {
    test.skip(target.versions.length < 2, "needs 2+ versions to switch");
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto(EVERYTHING); // e.g. /test/v2/everything/
    const toggle = () =>
      page.evaluate(() => (window as unknown as { toggleMobileSidebar: () => void }).toggleMobileSidebar());
    const panel = page.locator(".sidebar-mobile-panel");
    const active = panel.locator(".sidebar-mobile-version-active");

    await toggle();
    const ownVersionHref = await active.getAttribute("href");
    await panel
      .locator(`.sidebar-mobile-version-link[href*="/${FIRST_VERSION}/"]`)
      .first()
      .click();
    await expect(active).toHaveAttribute("href", new RegExp(`/${FIRST_VERSION}/`));

    // Close without picking a topic, then reopen.
    await toggle();
    ...
    ).toHaveAttribute("href", ownVersionHref!);
    // And the restored drawer is live, not inert markup: swapping again works.
    await panel
      .locator(`.sidebar-mobile-version-link[href*="/${FIRST_VERSION}/"]`)
      .first()
      .click();
  ...
  // bindDrawer re-runs on every swap, and wireScroller registers a window
  // 'resize' listener plus a ResizeObserver per chip row. Those two are the
  // registrations that do NOT die when innerHTML is replaced — window outlives
  // the swap and an observer keeps its detached target alive — so without
  // explicit teardown they accumulate for as long as the reader keeps hopping
  // versions. Counting live ResizeObservers is the measurable proxy: it should
  // track the number of chip rows, not the number of swaps.
  test("repeated swaps do not accumulate observers", async ({ page }) => {
    test.skip(target.versions.length < 3, "needs 3+ versions for repeated hops");
    const hops = target.versions.filter((v) => !EVERYTHING.includes(`/${v}/`));
    await page.setViewportSize({ width: 390, height: 844 });
    // Patch before any page script runs so every construction is counted.
    await page.addInitScript(() => {
      const Native = window.ResizeObserver;
      let live = 0;
      (window as unknown as { __liveObservers: () => number }).__liveObservers = () => live;
      class Counting extends Native {
        private counted = false;
        observe(...args: Parameters<ResizeObserver["observe"]>) {
          if (!this.counted) { this.counted = true; live++; }
          return super.observe(...args);
        }
        disconnect() {
          if (this.counted) { this.counted = false; live--; }
    ...
    const active = panel.locator(".sidebar-mobile-version-active");
    const count = () =>
      page.evaluate(() => (window as unknown as { __liveObservers: () => number }).__liveObservers());

    const afterFirstBind = await count();
    expect(afterFirstBind, "no ResizeObserver was created — the probe is inert").toBeGreaterThan(0);

    // Hop back and forth several times; each hop re-binds the drawer.
    for (let i = 0; i < 4; i++) {
      const v = hops[i % hops.length];
      await panel.locator(`.sidebar-mobile-version-link[href*="/${v}/"]`).first().click();
      await expect(active).toHaveAttribute("href", new RegExp(`/${v}/`));
    }

    expect(
      await count(),
      "live ResizeObserver count grew across swaps — bindDrawer is not disconnecting the previous binding's observers",
    ).toBeLessThanOrEqual(afterFirstBind);
  });

  // A chip must always do something. When the fetch cannot complete, the handler
  // falls back to plain navigation rather than leaving the reader with a dimmed,
  // input-locked drawer and no way forward.
  test("a failed fetch falls back to navigating instead of stranding the drawer", async ({ page }) => {
    test.skip(target.versions.length < 2, "needs 2+ versions to switch");
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto(EVERYTHING);
    await page.evaluate(() => (window as unknown as { toggleMobileSidebar: () => void }).toggleMobileSidebar());
    ...
    await page.route(`**/${FIRST_VERSION}/**`, (route) =>
