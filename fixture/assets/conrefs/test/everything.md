...
### Uncapped table prose starves key column

The OPPOSITE failure from the table above, on the same uncapped 2-column shape, and the case the `anywhere` keyword cannot fix because it is the case `anywhere` causes. The table above pairs a long token with a short description, so the token pins column 1 open and starves the prose; the guard against that is the cell-level and `code`-level `overflow-wrap: anywhere` fold, which drops column 1's intrinsic min-content width to a single glyph so the auto layout can shrink it.

That fold has no lower bound. Invert the content ratio — a SHORT backticked key against a cell holding a paragraph of prose — and `table-layout: auto` hands the width out by max-content ratio, column 1 collapses all the way to its one-glyph floor, and the key renders as a vertical ribbon one character wide. No `max-width` is involved, which is why the 3-or-more-column `.table-capped` heuristic never fires here and why the "uncapped tables have a floor because nothing caps them" reasoning does not hold: the squeeze comes from the ratio, not from a cap.

Reproduces the production kgateway 2.3.x WAF IP-filtering page, whose cells measure 47/335 and 45/1047 characters. This is the ordinary `Setting | Description` field table that follows a configuration block in every task guide, so the shape is common rather than exotic. Measured here before the fix: 76px/595px at 1280px (11% of the row, 3 characters per line over 22 lines) and 54px/273px at 375px, at one character per line over 42 lines.

Neither `overflow-wrap` keyword can fix it, because the choice is between the two failures: `anywhere` is a ceiling with no floor, `break-word` a floor with no ceiling. The fix is a `min-width` floor on uncapped cells — 8rem, raised to 12rem at 768px and up — which bounds the collapse without touching the fold, so the ceiling the table above depends on survives. A percentage floor measured identical to no rule at all, because a percentage min-width resolves against the table's own width and the auto layout ignores it. `table-display.spec.ts` probes this at 1280px and 375px.

| Setting | Description |
| --- | --- |
| `MARKER_TABLE_UNCAPPED_PROSE.httpSettings.xffNumTrustedHops: 1` | Each proxy hop in the request chain appends an IP address to the `X-Forwarded-For` request header. Because of that, the header can hold multiple IP addresses in a comma-separated list. The gateway proxy uses this header to determine the number of trusted proxies that the request passed through and the value to use for the `REMOTE_ADDR` variable. The setting defines the number of IP addresses that the gateway needs to ignore to find the value for the `REMOTE_ADDR` variable, starting from the right. For example, setting this field to `1` means that the gateway proxy ignores the right-most entry and reads the entry immediately to its left as `REMOTE_ADDR`. The example in this guide assumes that the `X-Forwarded-For` request header contains two IP addresses: the IP address of the client that sent the request, such as your local machine, and the gateway proxy. A value of 1 means that the gateway proxy ignores the gateway proxy IP address and instead uses the client IP address for the `REMOTE_ADDR` variable. |
