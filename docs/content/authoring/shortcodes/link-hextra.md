...
Before reaching for it, read [Resolving links](../../links/). A plain
Markdown link is the default, and this shortcode is the exception for one
case: a path inside the current product and version tree. That page also
lists the call shapes that resolve to a real but wrong page instead of
failing.
